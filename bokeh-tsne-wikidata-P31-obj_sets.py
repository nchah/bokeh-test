...
def button_handler(event):
    ...
    sourcefile = "source1-tSNE-P31objset-" + s.value.replace(": ", "_").replace(", ", "-") + ".pickle"
    print('source1 data: ' + s.value)
    if runtime == 0:
        source1_data = pickle.load(open('source1-data-P31objset/' + sourcefile,'rb'))
        ...
        source1.data = pickle.load(open('source1-data-P31objset/' + sourcefile,'rb'))

    # Scatter plot
    p.scatter(source=source1, x="x", y="y", color="colors", fill_alpha=0.7, size=5)
    p.add_tools(HoverTool(tooltips=[("P31_obj_set (en)", "@P31_obj_set_id_label")]))
...
p = figure(title = "Clusters of P31_obj_set by their localization frequency across languages",
